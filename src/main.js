const VERSION_HISTORY = [
  {
    ...
    changes: [
      "Improved focus and keyboard handling across mode switching, adaptive nudges, delta review, archive jumps, and the Live Companion so rerenders stop feeling like they kicked the chair out from under the user.",
      "Tightened launch-readiness on mobile and accessibility details with better touch targets, skip navigation, stronger companion semantics, and reduced layout shift in heavy archive surfaces.",
      "Added performance-minded cleanup such as archive content visibility hints, more stable panel behavior, and clearer fallback copy so unfinished shell states stop pretending they are intentional."
    ]
  },
  {
    version: "v1.16.0",
    date: "2026-04-16",
    summary: "Added motion and interaction polish so the Raider Interface feels reactive, readable, and more product-grade under real use.",
    changes: [
      "Added restrained motion for system wake, feedback banners, mode shells, command selection, live companion expansion, and active-state pulses without turning the interface into a glow stick.",
      "Upgraded microinteractions across command tiles, pills, buttons, reviewed delta states, and readiness surfaces so state changes now feel confirmed instead of silent.",
      "Extended reduced-motion support so the new polish stays accessible and performance-safe on devices that should not be forced through theater."
    ]
  },
  {
    version: "v1.15.0",
    date: "2026-04-16",
    summary: "Added the adaptive behavior layer so the Raider Interface now remembers friction, unfinished pressure, and the next useful move.",
    changes: [
      "Added persistent adaptive state for mode history, repeated failure patterns, dismissed nudges, reviewed changes, and unresolved plan or blocker pressure.",
      "Updated the entry shell with state-driven recommendations, visible smart nudges, and reset or dismiss controls so returning users feel remembered without getting spammed.",
      "Extended the Live Run Companion and update tracking so active context, recovery pressure, and change-review memory now influence what the interface surfaces next."
    ]
  },
  {
    version: "v1.14.0",
    date: "2026-04-16",
    summary: "Demoted legacy browsing and reframed the old guide stack as contextual intelligence plus a secondary reference archive.",
    changes: [
      "Added contextual support cards inside the Raider Interface so lessons, machine intel, quests, materials, and updates now show up as supporting intelligence when the active mode actually needs them.",
      "Moved the old browse-first stacks behind a Reference Archive wrapper so deep panels remain available without defining the main product experience.",
      "Updated archive labels and interaction flow so mode actions can open the archive only when a deeper reference path is genuinely useful."
    ]
  },
  {
    version: "v1.13.0",
    date: "2026-04-16",
    summary: "Built the Live Run Companion overlay for fast plan recall before and during runs.",
    changes: [
      "Replaced the overlay placeholder with a globally accessible companion panel that works like a side drawer on desktop and a bottom sheet on mobile.",
      "Connected the overlay to active run-plan state so it can surface top priorities, the biggest warning, fallback guidance, quick reminders, and one-tap resume or adjust actions.",
      "Added compact and detailed states plus a useful empty state that routes players straight into planning when no active plan exists."
    ]
  },
  {
    version: "v1.12.0",
    date: "2026-04-16",
    summary: "Rebuilt Catch Me Up into a delta-driven mode that prioritizes behavior change over passive patch reading.",
    changes: [
      "Added delta logic that compares release signals against the user's last visit, then ranks what actually matters instead of mirroring a patch feed.",
      "Added actionable change cards with impact, response, ignore guidance, and direct handoff into the right mode or content lane.",
      "Added reviewed and revisit-later state so returning users can process update pressure without rereading the same board forever."
    ]
  },
  {
    version: "v1.11.0",
    date: "2026-04-16",
    summary: "Rebuilt Fix My Problem into a sharper recovery console with saved fixes, branching diagnosis, and direct handoff into planning.",
    changes: [
      "Expanded the problem system so players can diagnose early deaths, poor extractions, stash pressure, quest stalls, bad loadouts, confusion, and wasted runs without digging through generic guide copy.",
      "Added follow-up branching where it matters, then turned the output into one blunt correction, one safer path, and a cleaner route into the right system or mode.",
      "Added saved fixes and apply-plan handoff so recovery advice can move directly into Prep My Run instead of dying as a one-time suggestion."
    ]
  },
  {
    version: "v1.10.0",
    date: "2026-04-16",
    summary: "Added the core Raider Interface mode architecture across Brief Me, Prep My Run, Catch Me Up, and Fix My Problem.",
    changes: [
      "Built a shared mode shell with smooth switching, mode-aware state, and recoverable routing based on the current command.",
      "Gave each core mode its own layout behavior and output structure while keeping the system visually and architecturally connected.",
      "Kept the legacy guide sections available underneath so the new mode shell can route into existing content while the rewrite continues."
    ]
  },
  {
    version: "v1.9.0",
    date: "2026-04-15",
    summary: "Replaced the passive homepage opening with a reactive Raider Interface entry experience.",
    changes: [
      "Turned the Raider Interface shell into the new landing experience with direct command triggers for Brief Me, Prep My Run, Catch Me Up, and Fix My Problem.",
      "Added context-aware landing logic that reacts to prior visits, active plans, saved blockers, milestones, and patch drift.",
      "Kept the existing guide sections intact below the entry surface so the new landing flow can drive users into the right board without breaking the current app."
    ]
  },
  {
    version: "v1.7.0",
    date: "2026-04-15",
    summary: "Prepared the codebase for the Raider Interface rewrite without disrupting the live guide.",
    changes: [
      "Added rewrite scaffolding for the Raider Interface architecture including state, modes, command modeling, content delivery, and live companion placeholders.",
      "Exposed a runtime bridge so future phases can evolve current content and persistence models instead of rebuilding them in parallel.",
      "Kept the current field guide working while preparing a safer foundation for the full product rewrite."
    ]
  },
  {
    version: "v1.6.1",
...
function publishRuntimeBridge() {
  window.ARC_FIELD_GUIDE_RUNTIME = {
    app: {
      version: APP_VERSION,
      updated: APP_UPDATED,
      storageKey: STORAGE_KEY,
      name: "ARC Raiders Field Guide"
    },
    content: {
    ...
    state: {
      getSnapshot: createRuntimeSnapshot
    }
  };
}
